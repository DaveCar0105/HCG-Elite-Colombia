/*
class EmpaquePage extends StatefulWidget {
  @override
  _EmpaquePageState createState() => _EmpaquePageState();
}

class _EmpaquePageState extends State<EmpaquePage> {
  ...
  _guardarReporteEmpaque() async {
    ...
    empaque.empaqueFecha = '${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year}';
    ...
    if(empaque.empaqueId != null){
      await DatabaseEmpaque.updateEmpaque(empaque);
    }
    else{
      empaque.empaqueDesde = DateTime.now().millisecondsSinceEpoch;
      empaque.empaqueId = await DatabaseEmpaque.addEmpaque(empaque);
    }
  }
      ...
      child: RaisedButton(
        onPressed: () async{
          ...
          if(numeroOrden != '# ORDEN'
            ...
            Navigator.pushNamed(context,'falenciasCajaEmpaque',arguments: empaque);
          }else{
            _showSnackBar();
          }
        },
