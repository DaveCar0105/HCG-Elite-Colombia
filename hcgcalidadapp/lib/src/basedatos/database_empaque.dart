...
class DatabaseEmpaque{
  ...
  static Future<int> addEmpaques(ControlEmpaque empaque) async {
    ...
    '''INSERT INTO ${DatabaseCreator
        .controlEmpaqueTable}(${DatabaseCreator
        .empaqueNumeroOrden},${DatabaseCreator
        .empaqueTotal},${DatabaseCreator
        .empaqueFecha},${DatabaseCreator
        .empaqueAprobado},${DatabaseCreator
        .empaqueRamos},${DatabaseCreator
        .empaqueDespachar},${DatabaseCreator
        .empaqueDerogado},${DatabaseCreator
        .empaqueTallos},${DatabaseCreator
        .empaqueDesde},${DatabaseCreator
        .empaqueHasta},${DatabaseCreator
        .clienteId},${DatabaseCreator
        .productoId},${DatabaseCreator
        .postcosechaId},${DatabaseCreator
        .empaqueMarca},${DatabaseCreator
        .empaqueRamosRevisar}, ${DatabaseCreator
        .elite}) 
    VALUES('${empaque
        .empaqueNumeroOrden}',${empaque
        .empaqueTotal},'${empaque
        .empaqueFecha}',${empaque
        .empaqueAprobado},${empaque
        .empaqueRamos},${empaque
        .empaqueDespachar},'${empaque
        .empaqueDerogado}',${empaque
        .empaqueTallos},${empaque
        .empaqueDesde},${empaque
        .empaqueHasta},${empaque
        .clienteId},${empaque
        .productoId},${empaque
        .postcosechaId},'${empaque
        .empaqueMarca}',${empaque
        .ramosRevisar}, ${empaque
        .elite})''';
  ...
  static Future<int> addEmpaque(int controlEmpaqueId) async {

    final sql =
    '''INSERT INTO ${DatabaseCreator.empaqueTable} 
    (${DatabaseCreator.controlEmpaqueId})
    VALUES($controlEmpaqueId)''';
    return await db.rawInsert(sql);
  }
  static Future<List<Empaque>> getAllEmpaque(int controlEmpaqueId) async {
    final sql = '''SELECT * FROM ${DatabaseCreator.empaqueTable} WHERE ${DatabaseCreator.controlEmpaqueId} = $controlEmpaqueId''';
    final data =  await  db.rawQuery(sql);
    List<Empaque> empaques = List();
    for(final node in data){
      final sql1 = '''SELECT count(*) as FALENCIAS,MIN(${DatabaseCreator.falenciaEmpaqueId}) AS FALENCIA_ID FROM ${DatabaseCreator.falenciasReporteEmpaqueTable} WHERE ${DatabaseCreator.empaqueId} = ${node[DatabaseCreator.empaqueId]}''';
      final data1 =  await  db.rawQuery(sql1);

      final sql2 = '''SELECT * FROM ${DatabaseCreator.falenciaEmpaqueTable} WHERE ${DatabaseCreator.falenciaEmpaqueId} = ${data1[0]['FALENCIA_ID']}''';
      ...
      empaques.add(new Empaque(
          ...
          tipo:data2[0][DatabaseCreator.falenciaEmpaqueNombre].toString().substring(0,1)
      ));
    }
    return empaques;
  }
