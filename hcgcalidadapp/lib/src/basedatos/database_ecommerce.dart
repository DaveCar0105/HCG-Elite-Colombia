...
class DatabaseEcommerce{
  ...
  static Future<void> ecommerceSincronizados() async {
    final sql =
    '''UPDATE ${DatabaseCreator.controlEcommerceTable}
  ...
  static Future<int> addEcommerce(ControlRamos ramos) async {
    ...
    '''INSERT INTO ${DatabaseCreator
        .controlEcommerceTable}(${DatabaseCreator
  ...
  static Future<int> addProblemaEcommerce(ProblemaEcommerce problemaEcommerce) async {

    final sql =
    '''INSERT INTO ${DatabaseCreator
        .problemasEcommerceTable}(${DatabaseCreator
        .problemasEcommerceId},${DatabaseCreator
        .problemasEcommerceNombre},${DatabaseCreator
        .problemasEcommerceNumero},${DatabaseCreator
        .problemasEcommerceTipo}
    ) 
    VALUES(${problemaEcommerce
        .id},'${problemaEcommerce
        .nombre}',${problemaEcommerce
        .numero},${problemaEcommerce
        .tipo})''';
    return await db.rawInsert(sql);
  }

  static Future<int> updateProblemaEcommerce(ProblemaEcommerce problemaEcommerce) async {

    final sql =
    '''UPDATE ${DatabaseCreator
        .problemasEcommerceTable} SET 
        ${DatabaseCreator
        .problemasEcommerceNombre} = ${problemaEcommerce.nombre},
        ${DatabaseCreator
        .problemasEcommerceNumero} = ${problemaEcommerce.numero},
        ${DatabaseCreator
        .problemasEcommerceTipo} = ${problemaEcommerce.tipo}
    WHERE ${DatabaseCreator
        .problemasEcommerceId} = ${problemaEcommerce.id}
    ''';
    return await db.rawInsert(sql);
  }
  ...
  static Future<int> addCheckEcommerce(FalenciaReporteRamos falenciaReporteRamos) async {
  ...
  static Future<List<ProblemaEcommerce>> getProblemas()async{
    final sql =
    '''SELECT * FROM ${DatabaseCreator.problemasEcommerceTable}''';
    final data = await db.rawQuery(sql);
  ...
  static Future<List<ControlEcommerce>> getAllProblemasXEcommerceId(int id) async {
    final sql = '''SELECT ${DatabaseCreator.problemasEcommerceTable}.${DatabaseCreator.problemasEcommerceId},
    ...
    for(final node in data){
      listaCheck.add(new ControlEcommerce(
        ...
        noCumple: node[DatabaseCreator.checkEcommerceValor]==2?true:false,
        noAplica: node[DatabaseCreator.checkEcommerceValor]==3?true:false
      ));
    }
    return listaCheck;
  }
  ...
  static Future<void> updateProblemasXEcommerce(int id,int valor) async {
    ...
    final sql =
    '''UPDATE ${DatabaseCreator.checkEcommerceTable}
