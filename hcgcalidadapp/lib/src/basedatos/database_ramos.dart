...
class DatabaseRamos{
  ...
  static Future<int> addRamos(ControlRamos ramos) async {
    ...
    '''INSERT INTO ${DatabaseCreator
        .controlRamosTable}(${DatabaseCreator
  ...
  static Future<int> addRamo(int controlRamoId) async {

    final sql =
    '''INSERT INTO ${DatabaseCreator.ramosTable} 
    (${DatabaseCreator.controlRamosId})
    VALUES($controlRamoId)''';
    return await db.rawInsert(sql);
  }

  static Future<List<Ramo>> getAllRamo(int controlRamoId) async {
    final sql = '''SELECT * FROM ${DatabaseCreator.ramosTable} WHERE ${DatabaseCreator.controlRamosId} = $controlRamoId''';
    final data =  await  db.rawQuery(sql);
    List<Ramo> ramos = List();
    for(final node in data){
      final sql1 = '''SELECT count(*) as FALENCIAS FROM ${DatabaseCreator.falenciasReporteRamosTable} WHERE ${DatabaseCreator.ramosId} = ${node[DatabaseCreator.ramosId]}''';
