...
class DatabaseReportesAprobacion {
  static Future<List<ReporteAprobacionBanda>> getAllReportesBandas() async {
    ...
    while (data.length > 0) {
      ...
      while (subCliente.length > 0) {
        if (subCliente[0].containsKey('ramosTotal')) {
          ...
          final sql2 =
              '''SELECT COUNT(*) AS REPETIDOS, ${DatabaseCreator.falenciaBandaTable}.${DatabaseCreator.falenciaBandaId}, ${DatabaseCreator.falenciaRamosTable}.${DatabaseCreator.falenciaRamosNombre}
          ...
          AND ${DatabaseCreator.controlBandaTable}.${DatabaseCreator.ramosAprobado} IN (1, 4)
          GROUP BY ${DatabaseCreator.falenciaBandaTable}.${DatabaseCreator.falenciaBandaId}
  ...
  static Future<List<OrdenBanda>> getAllOrdenesBanda(int clienteId) async {
    ...
    ${DatabaseCreator.controlBandaTable}.${DatabaseCreator.ramoMarca},
    ${DatabaseCreator.controlBandaTable}.${DatabaseCreator.ramosTotal},
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      item.clienteNombre = data[i][DatabaseCreator.clienteNombre];
      item.tipoControlNombre = data[i][DatabaseCreator.tipoControlNombre];
      item.postCosechaNombre = data[i][DatabaseCreator.postcosechaNombre];
      item.productoNombre = data[i][DatabaseCreator.productoNombre];
      ...
      final sql1 = '''SELECT COUNT(*) AS RAMOS
          ...
          WHERE ${DatabaseCreator.bandaTable}.${DatabaseCreator.controlRamosId} = ${item.ordenRamoId}
          ''';
      var data1 = await db.rawQuery(sql1);
      try {
  ...
  static Future<Cantidad> getAllReportesCantidad() async {
    ...
    final sql8 = '''SELECT COUNT(*) AS CANTIDAD
          FROM ${DatabaseCreator.circuloCalidadTable}
          ''';
...
//eliminar depsues
  ...
  static circuloCalidadSinz() async {
    final sqlA = 'DELETE FROM ${DatabaseCreator.circuloCalidadTable}';
    await db.rawDelete(sqlA);
  }
